...
1. Monad Multisend dApp: A web-based decentralized application (located in the frontend folder) that allows users to send MON tokens from their wallet (via MetaMask) to multiple recipient addresses on the Monad Testnet in a single transaction. This utilizes the Multisend.sol smart contract.
2. MCP Server: A Node.js backend server (located in the backend folder) designed to act as a Model Context Protocol (MCP) server. It communicates via stdio and aims to provide information from the Monad Testnet (like balances and transaction details) to an MCP client such as Claude Desktop.
